...
func Submit(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"code": -1,
			"msg":  "read from request body error:" + err.Error(),
		})
		return
	}
	...
	err = models.DB.Where("problem_identity = ?", problemIdentity).Preload("TestCases").
		First(&pb).Error
	...
	if err = models.DB.Transaction(func(tx *gorm.DB) error {
		...
		// 更新 user_basic
		err = tx.Model(new(models.UserBasic)).Where("identity = ?", claims.Identity).Updates(m).Error
		if err != nil {
